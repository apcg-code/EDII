...
def pesquisa(nome, Ap, resultados=None):
    ...
    if Ap != None:
        ...
        while i < Ap.n:
            ...
            pesquisa(nome, Ap.p[i], resultados)
            registroA = Ap.r[i]
            print(registroA.Nome)
            print("procurando:")
            print(str(nome.Nome))
            if str(registroA.Nome).strip().lower() == str(nome.Nome).strip().lower():
                resultados.append(registroA.Elemento)
                print(registroA.Elemento)
...
def pesquisaInicial(nome, Ap, resultados=None):
    if resultados is None:
        resultados = []
    ...
    if Ap != None:
        ...
        while i < Ap.n:
            ...
            resultados = pesquisaInicial(nome, Ap.p[i], resultados)
            registroA = Ap.r[i]
            print(registroA.Nome)
            print("procurando:")
            print(str(nome.Nome))
            if str(registroA.Nome).strip().lower().startswith(str(nome.Nome).strip().lower()):
                resultados.append(registroA.Elemento)
                print("achei este:")
                print(registroA.Nome)
...
def _InserirElementos(Ap, ordem, dataframe, chave):
    ...
    for i in range(tam_lin):
        reg = Registro()
        reg.Chave = chave_inicial
        reg.Elemento = i
        reg.Nome = dataframe.iloc[i, 1]

        Ap = _Insere(reg, Ap, ordem)
        chave_inicial += 1
    return Ap, chave_inicial
...
def listar(ap, dataframe):
    ...
    def Imprimir(ap, resultado, dataframe):
        if (ap != None):
            i = 0
            while i < ap.n:
                Imprimir(ap.p[i], resultado, dataframe)

                resultado.append(
                    f" Chave: {ap.r[i].Chave} - Nome: {dataframe.iloc[ap.r[i].Elemento, 1]} - Posicao: {ap.r[i].Elemento}")
                i += 1
            Imprimir(ap.p[i], resultado, dataframe)
    Imprimir(ap, resultado, dataframe)
    return resultado
